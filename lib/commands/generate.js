...
module.exports = function(options) {
  ...
  if (options.argv.remain[0] == 'list') {
    generateList();
    return;
  } else if (options.argv.remain[0] == 'md') {
    generateMarkdown();
    return;
  };
...
var generateMarkdown = function() {
  var dir = process.cwd();
  var list = require(path.join(dir, 'list.json'));
  var doc = '';
  ['basic', 'moderate', 'hard'].forEach(function(level) {
    doc += '### ' + level + '\n\n';
    var ids = Object.keys(list[level])
    ids.forEach(function(id) {
      var question = list[level][id]

      doc += id + '. [' + question + '](https://github.com/Rafe/code-warrior-questions/tree/master/' + level + '/' + id + ')\n';
    });
    doc += '\n';
  });
  console.log(doc);
